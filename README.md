...
A PWA base on VUE。源于社群里一句：需要规范提问的格式。可能已经有满足类似需求的应用(没错儿，我发现了[nodebb](https://nodebb.org/)应该就是这么个玩意儿)，可**我偏偏不喜欢（其实是喜欢的，只是不想仅止于会用），就想自己撸一个**。于是决定抡起袖子开干，参考X乎。问答、聊天、文章聚合...（感觉装X有点过头了，先就这样吧...匿了）
